...
import electron, { ipcMain as ipcMain$1, session, BrowserView, app as app$1, BrowserWindow } from "electron";
...
class StoreManager {
  ...
  setTelegramInstances(instances) {
    this.store.set("telegrams", instances);
  }
  addTelegramInstance(instance) {
    const current = this.getTelegramInstances();
    current.push(instance);
    this.setTelegramInstances(current);
  }
  removeTelegramInstance(id2) {
    const current = this.getTelegramInstances();
    this.setTelegramInstances(current.filter((t2) => t2.id !== id2));
  }
  // Alias for ViewManager convenience
  removeTelegram(id2) {
    this.removeTelegramInstance(id2);
  }
  togglePin(id2) {
    const current = this.getTelegramInstances();
    const updated = current.map(
      (t2) => t2.id === id2 ? { ...t2, isPinned: !t2.isPinned } : t2
    );
    this.setTelegramInstances(updated);
  }
  renameTelegram(id2, newLabel) {
    const current = this.getTelegramInstances();
    const updated = current.map(
      (t2) => t2.id === id2 ? { ...t2, label: newLabel } : t2
    );
    this.setTelegramInstances(updated);
  }
  // Window State
  ...
  updateSettings(partial) {
    var _a;
    if (((_a = partial.sidebar) == null ? void 0 : _a.width) !== void 0) {
      this.setSidebarWidth(partial.sidebar.width);
    }
    if (partial.telegrams !== void 0) {
      this.setTelegramInstances(partial.telegrams);
    }
    if (partial.window !== void 0) {
...
class ViewManager {
  constructor(window) {
    ...
    __publicField(this, "_currentViewId", null);
    __publicField(this, "sidebarWidth", 300);
    __publicField(this, "preloadPath");
    this.window = window;
    this.preloadPath = path$1.join(__dirname$2, "preload.mjs");
    this.setupIPC();
  ...
  setupIPC() {
    ipcMain$1.on("switch-view", (_, viewId) => {
      this.switchView(viewId);
    });
    ipcMain$1.on("update-sidebar-width", (_, width) => {
      this.sidebarWidth = width;
      this.updateAllViewsBounds();
    });
    ipcMain$1.on("menu:refresh", (_, id2) => {
      this.refreshView(id2);
    });
    ipcMain$1.on("menu:hibernate", (_, id2) => {
      this.hibernateView(id2);
    });
    ipcMain$1.on("menu:delete", (_, id2) => {
      this.deleteView(id2);
    });
    ipcMain$1.on("menu:pin", (_, id2) => {
      storeManager.togglePin(id2);
    });
    ipcMain$1.on("menu:rename", (_, { id: id2, label }) => {
      storeManager.renameTelegram(id2, label);
    });
  }
  refreshView(id2) {
    ...
    if (view) {
      ...
      console.log(`[ViewManager] Refreshed view: ${id2}`);
    }
  }
  hibernateView(id2) {
    const view = this.views.get(id2);
    if (view) {
      if (this._currentViewId === id2) {
        this.window.setBrowserView(null);
        this._currentViewId = null;
      }
      view.webContents.close();
      this.views.delete(id2);
      console.log(`[ViewManager] Hibernated view: ${id2}`);
    }
  }
  deleteView(id2) {
    this.hibernateView(id2);
    storeManager.removeTelegram(id2);
    console.log(`[ViewManager] Deleted view: ${id2}`);
  }
  /**
   ...
   */
  createView(id2, url, partition) {
    if (this.views.has(id2)) return;
    ...
    console.log(`[ViewManager] Created view '${id2}' with partition '${partition}'`);
  }
  switchView(id2) {
    if (id2 === "home" || id2 === "settings" || id2 === "custom") {
      this.window.setBrowserView(null);
      this._currentViewId = null;
      return;
    }
    if (!this.views.has(id2)) {
      if (id2.startsWith("telegram")) {
        const instances = storeManager.getTelegramInstances();
        const instance = instances.find((t2) => t2.id === id2);
        if (instance) {
          this.createView(id2, "https://web.telegram.org/a/", instance.partition);
        } else {
          console.warn(`[ViewManager] Unknown telegram instance: ${id2}, using default partition`);
          this.createView(id2, "https://web.telegram.org/a/", `persist:${id2}`);
        }
      }
    }
    const targetView = this.views.get(id2);
    if (targetView) {
      ...
      this._currentViewId = id2;
    }
  }
}
app$1.disableHardwareAcceleration();
app$1.commandLine.appendSwitch("disable-gpu");
...
function startPythonServer() {
  ...
  pythonProcess.on("exit", (code2) => {
    safeLog(`[Electron] Python process exited with code ${code2}`);
  });
}
app$1.on("will-quit", () => {
  if (pythonProcess) {
...
ipcMain$1.handle("set-settings", (_, settings) => {
  storeManager.updateSettings(settings);
  return { success: true };
});
ipcMain$1.handle("translate", async (_, { text, targetLang }) => {
  try {
...
ipcMain$1.handle("get-governance-config", async () => {
  try {
